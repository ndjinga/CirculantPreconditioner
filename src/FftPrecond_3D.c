...
PetscErrorCode build_diag_mat_vec_3D(Vec c, PetscInt n_x, PetscInt n_y, PetscInt n_z, PetscScalar lambda_x, PetscScalar lambda_y, PetscScalar lambda_z) {
    PetscFunctionBeginUser;
    Mat FFT_cx, FFT_cy, FFT_cz;
    Vec c_x, c_y, c_z, c_x_hat, c_y_hat, c_z_hat, sum;
    ...
    build_circulant_col_1D(c_z, n_z);
    ...
    VecDuplicate(c, &kpi_y_intermediate);
    VecDuplicate(c, &kpi_z);
...
PetscErrorCode test_3D() {
    ...
    // Solve the system
    build_diag_mat_vec_3D(Diag, n_x, n_y, n_z, lambda_x, lambda_y, lambda_z);
